...
def eval_edge_predictions(p, database, save_dir):
    ...
    #files = glob.glob(os.path.join(save_dir, 'edges/*png'))
    ...
    os.system('rsync -a %s %s' %(os.path.join(save_dir, 'edges/*'), result_dir))
    print('Done ...')
...
if __name__ == '__main__':
    ...
    for i, pred_pth in enumerate(edge_pred_paths):
        if not os.path.isdir(pred_pth):
            print(f"\nSkipped...not a dir...{pred_pth}\n\n")
            continue
        start_time = time.time()
        print("---------------------------------------------------------------")
        print(f"[{i}] Evaluating edge predictions in {pred_pth}")
        print("---------------------------------------------------------------\n")

        eval_edge_predictions({'setup':'1','model':'model'}, 'PASCALContext', pred_pth)

        print("---------------------------------------------------------------")
        needed_time = (time.time() - start_time)
        print(f"[{i}] Evaluation took {needed_time:.2f}s")
        print("---------------------------------------------------------------\n\n\n")
